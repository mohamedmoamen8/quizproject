...
science_app = Blueprint('science_app', __name__, template_folder='templates', static_folder='static')
...
def load_questions():
    try:
        ...
        json_file_path = os.path.join(os.path.dirname(__file__), '..', 'json', 'science_questions.json ')
...
@science_app.route('/')
def index():
    return render_template('science.html')  # Template should be inside science_app/templates/



# API endpoint to get random questions
@science_app.route('/api/questions')
def questions():
